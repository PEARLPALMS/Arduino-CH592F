...
static CONST uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "CH592F HID Key";
...
extern void    hidEmuSendKbdReport(uint8_t keycode);
extern void    hidEmuSendKbdReport2(uint8_t keycode);
...
uint16_t HidEmu_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & START_REPORT_EVT){
      ...
      if(GPIOB_ReadPortPin(GPIO_Pin_12) == 0){
        ...
        while(GPIOB_ReadPortPin(GPIO_Pin_12) == 0){
          delay(1);
          i++;
        }
        if(i > 500)
          hidEmuSendKbdReport2(HID_KEYBOARD_A);
        else
          hidEmuSendKbdReport(HID_KEYBOARD_A);
        hidEmuSendKbdReport(0x00);
      // Button 2
      }else if(GPIOB_ReadPortPin(GPIO_Pin_7) == 0){
        GPIOA_ResetBits(GPIO_Pin_8);
        while(GPIOB_ReadPortPin(GPIO_Pin_7) == 0){
          delay(1);
          i++;
        }
        if(i > 500)
          hidEmuSendKbdReport2(HID_KEYBOARD_B);
        else
          hidEmuSendKbdReport(HID_KEYBOARD_B);
        hidEmuSendKbdReport(0x00);
      // Button 3
        ...
        if(i > 500)
          hidEmuSendKbdReport2(HID_KEYBOARD_C);
        else
          hidEmuSendKbdReport(HID_KEYBOARD_C);
        hidEmuSendKbdReport(0x00);
      }else{
        GPIOA_SetBits(GPIO_Pin_8);
      }

      tmos_start_task(hidEmuTaskId, START_REPORT_EVT, 0);
      return (events ^ START_REPORT_EVT);
    }
    return 0;
}
void sendStrKey(char *s){
  // for(int i = 0; i < sizeof(s); i++){
  for(int i = 0; i < 12; i++){
    // hidEmuSendKbdReport(HID_KEYBOARD_0);
    char2keycode(s[i]);
  hidEmuSendKbdReport(0x00);
      tmos_start_task(hidEmuTaskId, START_REPORT_EVT, 0);
  }
 ...
 * @return  none
 */
void hidEmuSendKbdReport(uint8_t keycode)
{
...
void hidEmuSendKbdReport2(uint8_t keycode)
{
    uint8_t buf[HID_KEYBOARD_IN_RPT_LEN];

    buf[0] = 0x02;    // Modifier keys
    buf[1] = 0;       // Reserved
    buf[2] = keycode; // Keycode 1
    buf[3] = 0;       // Keycode 2
    buf[4] = 0;       // Keycode 3
